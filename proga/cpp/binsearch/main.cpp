...
public:
  ...
  string left_name() {return left->name;} string right_name() {return right->name;}
};
...
Student* create_tree(int l, int r) {
...
int main() {
  ...
  Student* r = create_tree(0, students.size());
  //print_students_by_value(students);
  std::cout << students[23].left->name << std::endl;
